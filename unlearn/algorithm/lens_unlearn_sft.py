...
This version supports full SFT (no LoRA) or configurable LoRA rank.
"""
...
class RRTrainer(UnlearningTrainer):
    ...
    def __init__(self, *args, is_peft_model=True, **kwargs):
    ...
    def compute_loss(
        ...
        if self.is_peft_model:
            ...
            # For SFT: compute reference hidden states without gradient
            # This gives us a target to stay close to during training
            unwrapped_model.eval()  # type: ignore
            with torch.no_grad():
                if retain_coeff > 0:
                    orig_retain_outputs = unwrapped_model(**retain_inputs_dict)[module]  # type: ignore
...
if __name__ == "__main__":
    ...
    if args.lora:
        ...
        lora_config = LoraConfig(
            r=args.lora_r,
            lora_alpha=args.lora_r,  # Scale alpha with rank
            target_modules=(
                [
                    "q_proj",
                    "k_proj",
    ...
    trainer = RRTrainer(
        ...
        lens=lens, is_peft_model=args.lora
    )
    ...
    if args.save_name:
        ...
        args.save_name = f"{args.save_name}_{mode_str}"
